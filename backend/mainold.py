...
async def text_to_ai(request: Text2AiRequest):
    ...
    Language behavior:
    - If target_language is provided: Forces output in that language.
    - If None: Detects and uses the input text's language.
    """
    start = time.perf_counter()
    
    lang_directive = f"Output all generated values in {request.target_language}." if request.target_language else "Output all generated values in the same language as the input text."
    ...
    {lang_directive}
    IMPORTANT: Keep all JSON keys strictly in English (e.g., "summary", "entities"). Only the values should be translated/generated in the target language.
    
    Return a JSON object with:
    - "summary": A concise summary.
    - "entities": A list of objects {{ "name": "...", "type": "PERSON/ORG/DATE/LOC" }}.
...
async def page_to_ai(
    ...
    Extracts text/visuals and performs cognitive analysis (Summary, Key Points).
    """
    start = time.perf_counter()
    ...
    lang_directive = f"Output all generated values in {target_language}." if target_language else "Output all generated values in the same language as the document text."

    prompt = f"""
    Analyze the content extracted from a document page (Page Ref: {metadata_page_number}).
    
    {lang_directive}
    IMPORTANT: Keep all JSON keys strictly in English (e.g., "page_summary", "key_points"). Only the content values should be in the target language.
    ...
    if len(raw_text) < 100:
        img = get_page_image(doc)
        prompt = f"Analyze this document page image (Page Ref: {metadata_page_number}). Return JSON with page_summary, key_points, entities. {lang_directive} IMPORTANT: Keep JSON keys in English."
...
async def page_to_table(
    ...
    lang_directive = f"If any text descriptions are extracted/generated, ensure they are in {target_language}." if target_language else "Keep text descriptions in the document's original language."
